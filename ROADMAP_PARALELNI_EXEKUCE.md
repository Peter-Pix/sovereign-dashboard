...
- [x] E2E manuální test: hromadná exekuce tasků projektu (≥3 tasků) → sloty, disabled tlačítka; dokončení → uvolnění
  ...
  - ⚠️ **Adaptivní řízení (záměrné)**: hromadná exekuce tasků JEDNOHO projektu (≥3 tasků) NEJDE do 3/3 — tasky z jednoho projektu běží sériově (`MAX_PER_PROJECT=1`, konkurují si na souborech). Slots 2/3 max (1/projekt), nikdy 3 z jednoho projektu.
